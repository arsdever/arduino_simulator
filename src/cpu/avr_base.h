...
class CPU_EXPORT CAVRBase : public CCPU
{
	struct SAVRState : public CCPU::SState
	{
		enum EFlag
		{
			CFlag = 1,
		...
		SAVRState(quint32 memory_size)
			: PC{ 0 }
			, IR{ 0 }
			, DATA_MEMORY(new quint8[memory_size])
			, GR(DATA_MEMORY)
			, IOR(DATA_MEMORY + 32)
		...
		bool GetFlag(quint32 flag) const override { return FLAGS & flag; }
		void SetFlag(quint32 flag, bool set) override { if (set) FLAGS |= flag; else FLAGS &= ~flag; }

		quint32 GetFlagRegister() const override { return FLAGS; }
		...
		quint16& X() { return (quint16&)GetGeneralRegisterSet()[26]; }
		quint16& Y() { return (quint16&)GetGeneralRegisterSet()[28]; }
		quint16& Z() { return (quint16&)GetGeneralRegisterSet()[30]; }

		quint8* InstructionRegister() const override { return (quint8*)IR; }
		quint32& ProgramCounter() const override { return (quint32&)PC; }

		bool IsRunning() const override { return RUN; }
	};	

public:
	CAVRBase(quint32 flash_size, quint32 memory_size, quint32 eeprom_size, quint32 ext_ioreg_count);
	~CAVRBase();

	void Init();
	void Step();

	CRAM* GetRam() const override;
	void SetRam(CRAM* ram) override;
	
	SState* GetState() const override;
	
	quint8* GetIOPorts() const override;

protected:
	void Fetch() override;
	quint8 Decode(quint16 instruction) override;
	quint8 Decode() override;
	quint8 Execute() override;
...
private:
	...
	typedef quint8(CAVRBase::*FunctionPointer)();
	typedef QMap<quint32, CAVRBase::FunctionPointer> InstructionMap;
